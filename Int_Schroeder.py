...
def integral_schroeder(señal, fs=44100):
    """
    ...
    td  = int(fs*len(señal))
    td_slice = slice(-1, None, -1)
    cumulative_sum = np.cumsum(señal[td_slice]**2)[::-1]
    sum_td = np.sum(señal[1:td+1]**2)
    E = 10 * np.log10(cumulative_sum / sum_td)
    return E
